import { themeInputs } from "modules/theming/themeInputs";
...
export const getBoxShadow = (colorInput: string) => ({
  bs1: boxShadowOffsets.bso1
    .map(convertOffsetsToBoxShadow(colorInput))
    .join(", "),
  bs2: boxShadowOffsets.bso2
    .map(convertOffsetsToBoxShadow(colorInput))
    .join(", "),
  bs3: boxShadowOffsets.bso3
    .map(convertOffsetsToBoxShadow(colorInput))
    .join(", "),
  bs4: boxShadowOffsets.bso4
    .map(convertOffsetsToBoxShadow(colorInput))
    .join(", "),
  bs5: boxShadowOffsets.bso5
    .map(convertOffsetsToBoxShadow(colorInput))
    .join(", ")
});
